...
no SCL/ACSI object model, no real MMS/GOOSE wire encoding, no multicast
(OC-IP-Stack has no broadcast/multicast address concept at all). It borrows
...
over-engineer for OC scale" design philosophy.

## Architecture

```
   HMI (MineOS)  <--MMS-lite (TCP)-->  SCADA (OpenOS)  <--MMS-lite (TCP)-->  IED (OpenOS)
                                              ^                                    |
                                              '-----------GOOSE-lite (UDP)---------'
```
...
- **IED** (`sas/ied/engine.lua`, `rc.d/iedd.lua`): one generic,
  ...
  program image runs on every IED; only the config differs.
...
(`sas/proto/framing.lua`); GOOSE is unicast-fan-out UDP with `stNum`/`sqNum`
change/retransmit counters (`sas/proto/goose.lua`), since OC-IP-Stack has no
multicast.
...
5. **Alarms.** Configure an `sas-scada.cfg` alarm condition; confirm it
...
6. **Comm loss/recovery.** Stop `ipstackd` (or unplug the modem) on IED A.
...
7. **HMI.** Before GUI polish: confirm `require("ipstack.socket")` and a
